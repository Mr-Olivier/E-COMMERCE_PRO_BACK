...
class EmailService {
    constructor() {
        ...
        this.transporter = nodemailer_1.default.createTransport(Object.assign({ host: process.env.EMAIL_HOST, port: parseInt(process.env.EMAIL_PORT || "587"), secure: process.env.EMAIL_PORT === "465", auth: {
    ...
    sendVerificationEmail(email, firstName, otp) {
        ...
        `,
                ...
                if (process.env.NODE_ENV === "development") {
                    console.log(`⚠️ Email would have been sent to ${email} with OTP: ${otp}`);
                    return;
                }
    ...
    sendPasswordResetEmail(email, firstName, otp) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // Log the OTP for development purposes only
                if (process.env.NODE_ENV === "development") {
      ...
      `,
                ...
                // For development, don't throw the error
                if (process.env.NODE_ENV === "development") {
                    console.log(`⚠️ Password reset email would have been sent to ${email} with OTP: ${otp}`);
                    return;
                }
